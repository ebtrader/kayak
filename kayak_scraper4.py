...
options.binary_location = 'C:/Users/Owner/PycharmProjects/chromedriver_win32/chromedriver.exe'
            ...
            try:
                ...
                if times == []:
                    ...
                    try:
                        ...
                        rating = str(flight_info.find('span', {'class': 'details-holder'}).find('span', {
                            'class': 'route-happy-superlative'}).find('a').string)
                        satisfactory = rating[:rating.index('F') - 1]
                    except Exception as e:
                        continue
                    data_storage.write(
                        '\t'.join([depart_time, return_time, carrier, duration, price, satisfactory]) + '\n')
