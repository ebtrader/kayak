...
def scrape_kayak(start='', end='', airport='OPO'):
    ...
    if res:
        # print(res)
        url = "https://www.kayak.com/s/horizon/exploreapi/destinations?airport=" + airport + "&budget=&depart=" + start + "&return=" + end + "&tripdurationrange=4%2C7&duration=&flightMaxStops=&stopsFilterActive=false&topRightLat=51.82490080841914&topRightLon=8.962652968749989&bottomLeftLat=28.636584579286538&bottomLeftLon=-26.32543296874999&zoomLevel=5&selectedMarker=&themeCode=&selectedDestination="
    else:
        url = "https://www.kayak.com/s/horizon/exploreapi/destinations?airport=" + airport + "&budget=&tripdurationrange=4%2C7&duration=&flightMaxStops=&stopsFilterActive=false&topRightLat=51.82490080841914&topRightLon=8.962652968749989&bottomLeftLat=28.636584579286538&bottomLeftLon=-26.32543296874999&zoomLevel=5&selectedMarker=&themeCode=&selectedDestination="

    response = requests.post(url).json()

    df = pd.DataFrame(
        columns=['City', 'Country', 'Duration', 'Price', 'Airline', 'Airport', 'Depart', 'Return', 'Link'])
...
for origin in origins:
    ...
    df = scrape_kayak(start=start, end=end, airport=origin)
